...
<?php
Class delegate_m extends CI_Model{
    ...
    public function production_summary($sdate,$edate,$associateID){
        ...
        FROM(
        SELECT SUM(CPMAmt)  as sum_CPMAmt,
    ...
    function fetch_projectlist(){
        $sql = "SELECT * FROM project WHERE status=1 AND Owner='".$this->session->userdata('userdata')[0]->ManagerID."'";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
...
//        $EC = $Numeration_Value[0]['TC'];
    ...
    function fetch_query_log_pending($team){
        $Login_data = $this->session->userdata('userdata');
        $sql = "SELECT `queryid`, AssociateName, `Payer`, `RequestedDate`, `RequestedBy`, `amtinq`, `comment`, `AssignedID`, `status` FROM `querylog`,associate WHERE associate.AssociateID = querylog.requestedby AND requestedby IN ('".$team."','".$Login_data[0]->ManagerID."') AND status=18";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
    ...
    function fetch_query_log_archive($team){
        $Login_data = $this->session->userdata('userdata');
        $sql = "SELECT `queryid`, AssociateName, `Payer`, `RequestedDate`, `RequestedBy`, `amtinq`, `comment`, `AssignedID`, `status` FROM `querylog`,associate WHERE associate.AssociateID = querylog.requestedby AND requestedby IN ('".$team."','".$Login_data[0]->ManagerID."') AND status=33";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
    function fetch_query_log_posted($team){
        $Login_data = $this->session->userdata('userdata');
        $sql = "SELECT `queryid`, AssociateName, `Payer`, `RequestedDate`, `RequestedBy`, `amtinq`, `comment`, `AssignedID`, `status` FROM `querylog`,associate WHERE associate.AssociateID = querylog.requestedby AND requestedby IN ('".$team."','".$Login_data[0]->ManagerID."') AND status=34";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
        ...
        $sql = "SELECT `missingid`, `project`.`ProjectName`, `Category`, `Payer`, `CheckNo`, `RequestedDate`, 
            ...
            status.*
             ...
             WHERE status.statusid=missing.Status AND 
                            ...
                            RequestedBy IN ('".$team."','".$Login_data[0]->ManagerID."')";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
    ...
    function fetch_missing_eob_archive($team){
        ...
        $sql = "SELECT `missingid`, `project`.`ProjectName`, `Category`, `Payer`, `CheckNo`, `RequestedDate`, `AssociateName`, missing.Status 
                ...
                WHERE project.ProjectID=missing.ProjectID AND associate.AssociateID = missing.RequestedBy AND RequestedBy IN ('".$team."','".$Login_data[0]->ManagerID."') AND missing.Status=34";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
    ...
    // Author: Anand Srivastava Date: 15-Mar-2019
    function saveMissing($missingid,$comment,$note,$status,$action,$metric,$assignedID){
        $Login_data = $this->session->userdata('userdata');
    ...
    // Author: Jagdish Pandre Date: 31-March-2019
    ...
    function add_missing($cpdipage, $website, $available, $mcomment, $checknum, $camt, $sdate, $MissingTL_ID, $WorkID_Query){
        ...
        $batchno = $WorkID_Query[0]['BatchNo'];
        $cpm = $WorkID_Query[0]['CPM'];
        $sql = "INSERT INTO `missing`(`ProjectID`, `Category`, `DepositDate`, `Amount`, `Payer`, `Encounter`, `CheckAmt`, `CheckDate`, `CheckNo`, `Website`, `Availibility`, `CPDIPage`, `RequestedDate`, `RequestedBy`, `Comment`, `AssignedID`, `Status`, `action`, `eta`, `location`) 
        VALUES ('$projectname','$categoryid','$depositedate','$amount','$batchno','$cpm','$camt','$sdate','$checknum','$website','$available','$cpdipage','$postdate','".$Login_data[0]->AssociateID."','$mcomment','$missingtl','18','0','0','0')";
        $this->db->query($sql);
        return true;
    }
