...
<?php
Class mteamlead_m extends CI_Model{
    ...
    public function migrationresearch($AssociateID){
        $Login_data = $this->session->userdata('userdata');
        $sql = "UPDATE `missing` SET `AssignedID`='".$Login_data[0]->AssociateID."' WHERE `AssignedID`='".$AssociateID."' AND `Status`!=34";
        $this->db->query($sql);
        return true;   
    }
     ############## MISSING EOB #######################
    ...
    function fetch_missing_eob_archive($team){
        ...
        $sql = "SELECT `missingid`, `project`.`ProjectName`, `Category`, `Payer`, `CheckNo`, `RequestedDate`, `AssociateName`, missing.Status 
                ...
                WHERE project.ProjectID=missing.ProjectID AND associate.AssociateID = missing.RequestedBy  AND missing.Status=34";
        $query = $this->db->query($sql);
        $row = $query->result_array(); 
        return $row;
    }
    function updateworkassignstatus($missingid,$assignedid){
     ...
     function updateworkassigns($missingid,$assignedid){
        $sql = "UPDATE `missing` 
                SET Status=10
                WHERE `missingid`='$missingid.'";
        $this->db->query($sql);
    ...
    public function production_summary($sdate,$edate,$associateID){
     ...
     // Author: Anand Srivastava Date: 15-Mar-2019
    function fetch_dashboard_data($Project_List_ID,$sdate,$edate,$teams){
        ...
        SELECT COUNT(*) as ACount, SUM(CheckAmt) as AAmount, 0 as ICount, 0 as IAmount, 0 as PCount, 0 as PAmount, 0 as RCount, 0 as RAmount  FROM missing WHERE RequestedDate BETWEEN  '$sdate' AND '$edate' AND missing.ProjectID IN ('".$Project_List_ID."') AND Status=10 AND missing.AssignedID IN ('".$teams."')
        UNION ALL
        SELECT 0 as ACount, 0 as AAmount, COUNT(*) as ICount, SUM(CheckAmt) as IAmount, 0 as PCount, 0 as PAmount, 0 as RCount, 0 as RAmount FROM missing WHERE RequestedDate BETWEEN  '$sdate' AND '$edate' AND missing.ProjectID IN ('".$Project_List_ID."') AND Status=17 AND missing.AssignedID IN ('".$teams."')
        UNION ALL
        SELECT 0 as ACount, 0 as AAmount, 0 as ICount, 0 as IAmount, COUNT(*) as PCount, SUM(CheckAmt) as PAmount, 0 as RCount, 0 as RAmount FROM missing WHERE RequestedDate BETWEEN  '$sdate' AND '$edate' AND missing.ProjectID IN ('".$Project_List_ID."') AND Status=18 AND missing.AssignedID IN ('".$teams."')
        UNION ALL
        SELECT 0 as ACount, 0 as AAmount, 0 as ICount, 0 as IAmount,0 as PCount, 0 as PAmount, COUNT(*) as RCount, SUM(CheckAmt) as RAmount FROM missing WHERE RequestedDate BETWEEN  '$sdate' AND '$edate' AND missing.ProjectID IN ('".$Project_List_ID."') AND Status=33 AND missing.AssignedID IN ('".$teams."')
