                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover" id="MissingEOB">
                                ...
                                <tbody>
                                    <?php
                                    for($i=0;$i<sizeof($missingeobpendingdata[0]);$i++){
                                        echo "<tr>";
                                            if($missingeobpendingdata[0][$i]['Status']==34){
                                                echo "<td>Posted</td>";
                                            }
                                            else
                                            {
                                                $missingid = base64_encode($missingeobpendingdata[0][$i]["missingid"]);
                                                $pageid = base64_encode('Missing_EOB_Pending');
                                                $missingIDpath = BASE_URL.'C_delegate/missingworking/'.$missingid.'/'.$pageid;
                                                echo "<td>".anchor($missingIDpath, 'Begin')."</td>";
                                            }
                                            echo "<td>".$missingeobpendingdata[0][$i]["missingid"]."</td>";
                                            echo "<td>".$missingeobpendingdata[0][$i]["ProjectName"]."</td>";
                                            echo "<td>".$missingeobpendingdata[0][$i]["Category"]."</td>";
                                            echo "<td>".$missingeobpendingdata[0][$i]["Payer"]."</td>";
                                            echo "<td>".$missingeobpendingdata[0][$i]["CheckNo"]."</td>";
                                            $date1= date("Y-m-d");
                                            $date2= date('Y-m-d',strtotime($missingeobpendingdata[0][$i]["RequestedDate"]));
                                            $diff=date_diff(date_create($date1),date_create($date2));
                                            echo "<td>".$diff->format("%a days")."</td>";
                                            echo "<td>".$missingeobpendingdata[0][$i]["RequestedByAssociateName"];
                                            ?>
                                            <a class="btn btn-info btn-xs" 
                                                data-toggle="modal" 
                                                data-target='#myModal' 
                                                onclick="getmissingid('<?php echo $missingeobpendingdata[0][$i]['missingid'];?>')">
