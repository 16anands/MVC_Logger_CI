...
class Usertracking{
    ...
    /* Constructor - Runs the initialization method. | @access public | @return void | boolean Will return FALSE on intialization failure.*/
    ...
    /*Initialization script - ensures library execution | @access public | boolean Will return FALSE on intialization failure.*/
    ...
    /* Auto-track is meant to be run only from a CodeIgniter hook. */
    ...
    /* Retrieves information from the session/user_agent/server_fields then adds a record.  */
    public function track_this(){
        ...
        //get the data
        $input_data = array();
        $input_data['session_id'] = $this->CI->session->userdata('__ci_last_regenerate');
        ...
        if ($this->configuration['user_identifier'] !== null && is_array($this->configuration['user_identifier'])){
            ...
            if ( $this->CI->load->model($class_name)){
    ...
    /*It processes any filters defined in the configuration file and returns either TRUE or FALSE.*/
    ...
    /*This checks database if defined in the configuration file
    *builds the necessary database tables for tracking using the CI database forge class.
    ...
    private function check_database(){
        ...
        if ($this->CI->db->table_exists('usertracking')){
            ...
            if ($num_matched < count($this->needed_fields) && $this->configuration['auto_fix_db'] === TRUE){
                ...
                $this->CI->dbforge->rename_table($db_prefix . 'usertracking', 'usertracking_backup_' . date('Y-m-d H:i:s'));
